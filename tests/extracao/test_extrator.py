...
from app.geradores.boleto_sintetico import formata_data, formata_moeda

GABARITO = json.loads(Path("dados/sinteticos/boletos/boleto-001.json").read_text("utf-8"))["campos"]

# As formas impressas vêm do próprio gabarito: o corpus é regerado, e um
# literal aqui passa a testar o valor de um boleto que não existe mais.
VALOR_IMPRESSO = formata_moeda(Decimal(GABARITO["valor"]))
VENCIMENTO_IMPRESSO = formata_data(date.fromisoformat(GABARITO["vencimento"]))
...
class TestConversao:
    ...
    def test_aceita_valor_na_forma_impressa(self) -> None:
        """O modelo devolve o que está na página: 3.112,30, não 3112.30."""
        dominio, erro = para_dominio(_bruto(valor=VALOR_IMPRESSO))

        assert "," in VALOR_IMPRESSO
        assert dominio is not None, erro

    def test_aceita_data_na_forma_impressa(self) -> None:
        dominio, erro = para_dominio(_bruto(vencimento=VENCIMENTO_IMPRESSO))

        assert VENCIMENTO_IMPRESSO.count("/") == 2
        assert dominio is not None, erro
